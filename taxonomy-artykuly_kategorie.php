					...
					<!-- .Page Banner  -->		
						...
						<?php 
							...
							<?php } else { ?>
			   			 		...
			   			 		<p><?php _e('Przepraszamy, aktualnie nie ma żadnych artykułów w tej kategorii. Pracujemy nad tym', 'mws'); ?></p>
							...
							<?php if ( !empty( $terms ) && is_array( $terms ) ) { 
								...
								<ul class="category__list">
								   <?php  foreach ( $terms as $term ) { ?>
								   		<li <?php if($queried_object->slug == $term->slug ) echo 'class="category--blue__item"';?>>
								        <a href="<?php echo esc_url( get_term_link( $term ) ) ?>" class="category--blue__link">
								            <?php echo $term->name; ?>
								        </a></li>
								    <?php } ?>
								     <li class="">
								    	<a href="<?php echo site_url('/artykuly');?>" class="category--blue__link">Wszystkie</a>
								    </li>
								</ul>
								<?php } ?>
						</nav>
